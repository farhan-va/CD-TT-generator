from tkinter import *
from datetime import datetime

win = Tk()
win.title("CD TT Filename Generator")
win.geometry('600x800')
win.configure(bg='#a5e8e5')


def onCopyClick():
    blankFlag = False
    invalidFlag = False
    SRFlag = True
    cdttStr = cdtt
    if ttNum.get() == "":
        blankFlag = True
    cdttStr += company.get() + ttNum.get()
    # checking for division for SQCC only
    if company.get() == "SQCC":
        if div.get() == "":
            blankFlag = True
        cdttStr += " " + div.get() + " "
    # amount formatting
    amountStr = ""
    foreignAmountStr = ""
    if jvNum.get() == "" or clientDocCode.get() == "" or ourDocCode.get() == "":
        blankFlag = True
    cdttStr += " SR " + amountStr + " " + jvNum.get() + " - "
    # outgoing transfers -ve, incoming +ve, paymentType format
    if foreignCur.get() != "SR":
        SRFlag = False
        if paymentType.get() == "Outgoing":
            if amount.get() == "":
                blankFlag = True
            elif foreignAmount.get() == "":
                blankFlag = True
            elif amount.get()[0] != '-':
                amountStr += '-'
                if foreignAmount.get()[0] != '-':
                    foreignAmountStr += '-'
            if not blankFlag:
                if any(c.isalpha() for c in amount.get()):
                    invalidFlag = True
                    invalidData('Amount')
                if any(c.isalpha() for c in foreignAmount.get()):
                    invalidFlag = True
                    invalidData('Foreign Amount')
                else:
                    amountStr += '{:,.2f}'.format(
                        float(amount.get().replace(",", "")))
                    foreignAmountStr += '{:,.2f}'.format(
                        float(foreignAmount.get().replace(",", "")))
        else:
            if amount.get() == "":
                blankFlag = True
            elif foreignAmount.get() == "":
                blankFlag = True
            else:
                # 123,456,789.01 format
                amountStr += '{:,.2f}'.format(float(amount.get().replace(",", "")))
                foreignAmountStr += '{:,.2f}'.format(
                    float(foreignAmount.get().replace(",", "")))
    else:
        if paymentType.get() == "Outgoing":
            if amount.get() == "":
                blankFlag = True
            elif amount.get()[0] != '-':
                amountStr += '-'
            if not blankFlag:
                if any(c.isalpha() for c in amount.get()):
                    invalidFlag = True
                    invalidData('Amount')
                else:
                    amountStr += '{:,.2f}'.format(
                        float(amount.get().replace(",", "")))
        else:
            if amount.get() == "":
                blankFlag = True
            else:
                # 123,456,789.01 format
                amountStr += '{:,.2f}'.format(
                    float(amount.get().replace(",", "")))
    if paymentType.get() == "Outgoing":
        cdttStr += "Vendor payable to "
    else:
        cdttStr += "Customer receivable from "
    if transactee.get() == "":
        blankFlag = True
    if not SRFlag:
        if frex.get() == "":
            blankFlag = True
    cdttStr += transactee.get() + " - " + clientDoc.get() + " " + clientDocCode.get() + " " + ourDoc.get() + \
        " " + ourDocCode.get() + " "
    if foreignCur.get() == "Other":
        if otherCur.get() == "":
            blankFlag = True
        else:
            cdttStr += otherCur.get() + " "
    else:
        cdttStr += foreignAmountStr + " "
    cdttStr += frex.get() + " "
    if any(c.isalpha() for c in clientDocCode.get()):
        invalidFlag = True
        invalidData('Client doc. no.')
    if any(c.isalpha() for c in ourDocCode.get()):
        invalidFlag = True
        invalidData('Our doc. no.')
    if purpose.get() == "":
        blankFlag = True
    cdttStr += purpose.get() + " "
    # date validation and correction
    dateStr = date.get()
    if dateStr == "":
        blankFlag = True
    else:
        try:
            dateObj = datetime.strptime(dateStr, "%d-%m-%Y")
            cdttStr += dateStr
        except ValueError:
            try:
                dateObj = datetime.strptime(dateStr, "%d/%m/%Y")
                dateFormatted = dateObj.strftime("%d-%m-%Y")
                cdttStr += dateFormatted
            except ValueError:
                invalidFlag = True
                invalidData('Date')
    # static length check
    if len(cdttStr) <= 255:
        if blankFlag:
            fieldLeftBlank()
        elif not invalidFlag:
            clipCopy(cdttStr)
    else:
        lengthOverflow()


def onResetClick():
    company.set(companyList[0])
    div.set(None)
    ttNumEntry.delete(0, END)
    amountEntry.delete(0, END)
    paymentType.set(paymentTypeList[0])
    jvNumEntry.delete(0, END)
    amountEntry.delete(0, END)
    transacteeEntry.delete(0, END)
    otherCurEntry.delete(0, END)
    clientDoc.set(clientDocList[0])
    ourDoc.set(ourDocList[0])
    clientDocCodeEntry.delete(0, END)
    ourDocCodeEntry.delete(0, END)
    foreignCur.set(foreignCurList[0])
    foreignAmountEntry.delete(0, END)
    frexEntry.delete(0, END)
    purposeEntry.delete(0, END)
    dateEntry.delete(0, END)
    resetLabel.configure(text="All fields cleared!")


def boxClear(event):
    event.widget.delete(0, "end")
    return None


def clipCopy(copyStr):
    win.clipboard_clear()
    win.clipboard_append(copyStr)
    win.update()
    outputLabel.configure(text="Generated and copied!", fg="dark green")


def lengthOverflow():
    outputLabel.configure(
        text="Max length of 255 characters exceeded!", fg="red")


def fieldLeftBlank():
    outputLabel.configure(text="Required field(s) left blank!", fg="red")


def invalidData(fieldName):
    outputLabel.configure(text="Invalid data in '" +
                          fieldName + "' field!", fg="red")


def clearLabel():
    outputLabel.configure(text="")
    resetLabel.configure(text="")


cdtt = "CD TT "
# Frame
frame = Frame(win, padx=7, pady=7, bg='#a5e8e5')
frame.place(relx=0.5, rely=0.5, anchor=CENTER)
# Header
headerLabel = Label(win, text="CD TT Filename Generator", font=(
    "Consolas", 18), bg='#a5e8e5', fg="dark blue", padx=10, pady=15)
headerLabel.place(relx=0.5, anchor=N)
# Footer
footerLabel = Label(
    win, text="Made by Farhan Arshad\nVersion 0.1", bg='#a5e8e5', fg="grey", padx=7, pady=7)
footerLabel.place(relx=1, rely=1, anchor=SE)
# Company
company = StringVar()
companyLabel = Label(frame, text="Company: ", pady=7, bg='#a5e8e5')
companyLabel.grid(row=0, column=0, sticky=E)
companyList = [
    "AR", "BA", "SQSS", "SQCC"
]
company.set(companyList[0])
companyRbuttons = []
companyRbuttonsFrame = Frame(frame, bg='#a5e8e5')
companyRbuttonsFrame.grid(row=0, column=1, sticky=W)
buttonListIndex = 0
for text in companyList:
    companyRbuttons.append(Radiobutton(
        companyRbuttonsFrame, text=text, variable=company, value=text, bg='#a5e8e5'))
    companyRbuttons[buttonListIndex].grid(
        row=0, column=buttonListIndex, sticky=W, padx=3)
    buttonListIndex += 1
# SQCC division
div = StringVar()
divLabel = Label(frame, text="Division: ", pady=7, bg='#a5e8e5')
divLabel.grid(row=1, column=0, sticky=E)
divList = [
    "WH", "FS", "IT", "ST", "TY"
]
div.set(None)
divRbuttons = []
divRbuttonsFrame = Frame(frame, bg='#a5e8e5')
divRbuttonsFrame.grid(row=1, column=1, sticky=W)
buttonListIndex = 0
for text in divList:
    divRbuttons.append(Radiobutton(
        divRbuttonsFrame, text=text, variable=div, value=text, bg='#a5e8e5'))
    divRbuttons[buttonListIndex].grid(
        row=1, column=buttonListIndex, sticky=W, padx=3)
    buttonListIndex += 1
# TT Code
ttNum = StringVar()
ttNumLabel = Label(frame, text="TT Code: ", bg='#a5e8e5')
ttNumLabel.grid(row=2, column=0, sticky=E)
ttNumEntry = Entry(frame, textvariable=ttNum, width=50)
ttNumEntry.grid(row=2, column=1, pady=3)
# Amount
amount = StringVar()
amountLabel = Label(frame, text="Amount in SR: ", bg='#a5e8e5')
amountLabel.grid(row=3, column=0, sticky=E)
amountEntry = Entry(frame, textvariable=amount, width=50)
amountEntry.grid(row=3, column=1, pady=3)
# Payment type
paymentType = StringVar()
paymentTypeLabel = Label(frame, text="Payment type: ", pady=7, bg='#a5e8e5')
paymentTypeLabel.grid(row=4, column=0, sticky=E)
paymentTypeList = [
    "Outgoing", "Incoming"
]
paymentType.set(paymentTypeList[0])
paymentTypeRbuttons = []
paymentTypeRbuttonsFrame = Frame(frame, bg='#a5e8e5')
paymentTypeRbuttonsFrame.grid(row=4, column=1, sticky=W)
buttonListIndex = 0
for text in paymentTypeList:
    paymentTypeRbuttons.append(Radiobutton(
        paymentTypeRbuttonsFrame, text=text, variable=paymentType, value=text, bg='#a5e8e5'))
    paymentTypeRbuttons[buttonListIndex].grid(
        row=4, column=buttonListIndex, sticky=W, padx=3)
    buttonListIndex += 1
# JV no.
jvNum = StringVar()
jvNumLabel = Label(frame, text="JV No.: ", bg='#a5e8e5')
jvNumLabel.grid(row=5, column=0, sticky=E)
jvNumEntry = Entry(frame, textvariable=jvNum, width=50)
jvNumEntry.grid(row=5, column=1, pady=3)
# Transactee
transactee = StringVar()
transacteeLabel = Label(frame, text="Vendor/Customer: ", bg='#a5e8e5')
transacteeLabel.grid(row=6, column=0, sticky=E)
transacteeEntry = Entry(frame, textvariable=transactee, width=50)
transacteeEntry.grid(row=6, column=1, pady=3)
# Client doc
clientDoc = StringVar()
clientDocLabel = Label(frame, text="Client doc.: ", pady=7, bg='#a5e8e5')
clientDocLabel.grid(row=7, column=0, sticky=E)
clientDocList = [
    "Inv", "PO", "Qtn"
]
clientDoc.set(clientDocList[0])
clientDocRbuttons = []
clientDocRbuttonsFrame = Frame(frame, bg='#a5e8e5')
clientDocRbuttonsFrame.grid(row=7, column=1, sticky=W)
buttonListIndex = 0
for text in clientDocList:
    clientDocRbuttons.append(Radiobutton(
        clientDocRbuttonsFrame, text=text, variable=clientDoc, value=text, bg='#a5e8e5'))
    clientDocRbuttons[buttonListIndex].grid(
        row=7, column=buttonListIndex, sticky=W, padx=3)
    buttonListIndex += 1
# Client doc code
clientDocCode = StringVar()
clientDocCodeLabel = Label(frame, text="Client doc. no.: ", bg='#a5e8e5')
clientDocCodeLabel.grid(row=8, column=0, sticky=E)
clientDocCodeEntry = Entry(frame, textvariable=clientDocCode, width=50)
clientDocCodeEntry.grid(row=8, column=1, pady=3)
# Our doc
ourDoc = StringVar()
ourDocLabel = Label(frame, text="Our doc.: ", pady=7, bg='#a5e8e5')
ourDocLabel.grid(row=9, column=0, sticky=E)
ourDocList = [
    "PO", "RFQ", "SO"
]
ourDoc.set(ourDocList[0])
ourDocRbuttons = []
ourDocRbuttonsFrame = Frame(frame, bg='#a5e8e5')
ourDocRbuttonsFrame.grid(row=9, column=1, sticky=W)
buttonListIndex = 0
for text in ourDocList:
    ourDocRbuttons.append(Radiobutton(
        ourDocRbuttonsFrame, text=text, variable=ourDoc, value=text, bg='#a5e8e5'))
    ourDocRbuttons[buttonListIndex].grid(
        row=9, column=buttonListIndex, sticky=W, padx=3)
    buttonListIndex += 1
# Our doc code
ourDocCode = StringVar()
ourDocCodeLabel = Label(frame, text="Our doc. no.: ", bg='#a5e8e5')
ourDocCodeLabel.grid(row=10, column=0, sticky=E)
ourDocCodeEntry = Entry(frame, textvariable=ourDocCode, width=50)
ourDocCodeEntry.grid(row=10, column=1, pady=3)
# Foreign currency
foreignCur = StringVar()
foreignCurLabel = Label(frame, text="Foreign Currency: ", pady=7, bg='#a5e8e5')
foreignCurLabel.grid(row=11, column=0, sticky=E)
foreignCurList = [
    "SR" ,"USD", "GBP", "AED", "INR", "Other"
]
foreignCur.set(foreignCurList[0])
foreignCurRbuttons = []
foreignCurRbuttonsFrame = Frame(frame, bg='#a5e8e5')
foreignCurRbuttonsFrame.grid(row=11, column=1, sticky=W)
buttonListIndex = 0
for text in foreignCurList:
    foreignCurRbuttons.append(Radiobutton(
        foreignCurRbuttonsFrame, text=text, variable=foreignCur, value=text, bg='#a5e8e5'))
    foreignCurRbuttons[buttonListIndex].grid(
        row=11, column=buttonListIndex, sticky=W, padx=3)
    buttonListIndex += 1
# Other cur
otherCur = StringVar()
otherCurLabel = Label(frame, text="If other, specify: ", bg='#a5e8e5')
otherCurLabel.grid(row=12, column=0, sticky=E)
otherCurEntry = Entry(frame, textvariable=otherCur, width=50)
otherCurEntry.grid(row=12, column=1, pady=3)
# Foreign amount
foreignAmount = StringVar()
foreignAmountLabel = Label(frame, text="Foreign Amount: ", bg='#a5e8e5')
foreignAmountLabel.grid(row=13, column=0, sticky=E)
foreignAmountEntry = Entry(frame, textvariable=foreignAmount, width=50)
foreignAmountEntry.grid(row=13, column=1, pady=3)
# Frex
frex = StringVar()
frexLabel = Label(frame, text="Frex: ", bg='#a5e8e5')
frexLabel.grid(row=14, column=0, sticky=E)
frexEntry = Entry(frame, textvariable=frex, width=50)
frexEntry.grid(row=14, column=1, pady=3)
# Purpose
purpose = StringVar()
purposeLabel = Label(frame, text="Purpose: ", bg='#a5e8e5')
purposeLabel.grid(row=15, column=0, sticky=E)
purposeEntry = Entry(frame, textvariable=purpose, width=50)
purposeEntry.grid(row=15, column=1, pady=3)
# Date
date = StringVar()
dateLabel = Label(frame, text="Date (DD-MM-YYYY): ", bg='#a5e8e5')
dateLabel.grid(row=16, column=0, sticky=E)
dateEntry = Entry(frame, textvariable=date, width=50)
dateEntry.grid(row=16, column=1, pady=3)

# Execution button
copyButton = Button(
    frame, text="Generate and copy to clipboard", command=onCopyClick, bg='white')
copyButton.grid(row=17, column=0, pady=10)
# Result indicator
outputLabel = Label(frame, text="", pady=10, bg='#a5e8e5')
outputLabel.grid(row=17, column=1)
win.after(5000, clearLabel)
# Reset button
resetButton = Button(frame, text="Reset fields",
                     command=onResetClick, bg='white')
resetButton.grid(row=18, column=0, pady=5)
# Reset indicator
resetLabel = Label(frame, text="", pady=5, bg='#a5e8e5')
resetLabel.grid(row=18, column=1)

win.mainloop()